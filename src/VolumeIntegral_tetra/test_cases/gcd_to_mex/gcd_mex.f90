...
! Load the output 1 into a MATLAB array.
      g_pr = mxGetPr(plhs(1))
      siz=mo*no
      call mxCopyReal8ToPtr(dble(g), g_pr, siz)
      if(debu) write(66,*) 'G'
	  deallocate(g)
...
! Load the output2 into a MATLAB array.
      ...
      call mxCopyReal8ToPtr(dble(c), c_pr, siz)
      if(debu) write(66,*) 'C'
	  deallocate(c)
...
! Load the output 3 into a MATLAB array.
      d_pr = mxGetPr(plhs(3))
      siz=mo*no
      call mxCopyReal8ToPtr(dble(d), d_pr, siz)
      if(debu) write(66,*) 'D'
	  deallocate(d)
...
! Load the output2 into a MATLAB array.
      ...
      call mxCopyReal8ToPtr(dble(f(1:5,1:nf)), f_pr, siz)
      if(debu) write(66,*) 'F'	  
      deallocate(f)
