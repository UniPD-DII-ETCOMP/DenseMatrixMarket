...
real*8 barhh(3),barkk(3)
real*8 X1,Y1,Z1,WW1
real*8 Xke1(nv_max)
real*8 Yke1(nv_max)
real*8 Zke1(nv_max)
real*8 Wke1(nv_max), distt
...
!$OMP PARALLEL SHARED(N_edge,Matrix_P0,Matrix_L,n_EV,EV,nv_max,VP,EV_ind,Neps, &
!$OMP                 Nhh,hhout,Nkk,kkout,bared,thr)
...
!$OMP                                L,ii,jj,dist,hhloc,kkloc,barhh,barkk, &
!$OMP                                X1,Y1,Z1,WW1,Xke1,Yke1,Zke1,Wke1,distt)
do hhloc = 1,Nhh
	hh=hhout(hhloc)
    ...
    do kkloc = 1,Nkk
		...
		barkk=bared(1:3,kk)
		call norm_eps(barhh-barkk,0.0d0,distt)
		if(distt<thr) then
			do ee=1,nv_kk !ciclo sui volumi comuni lato kk
				pvt(1:3,1:4)=Matrix_P0(1:3,VP(1:4,idV_kk(ee))); ! punti del tetra
        ...
        do ee=1,nv_kk !ciclo sui volumi comuni lato kk
				...
				call tetareavol(pvt,nnorm,area,vol_kk(ee)); !trovo le grandezze del tetraedro
				call fun_whitney_tetra_rot_edge(nnorm,area,vol_kk(ee),EV_ind(ee,kk),w_kk(1:3,ee)); !trovo le whitney del lato hh nel volume qq
				call fun_tetra_one_point(pvt,vol_kk(ee),X1,Y1,Z1,WW1);! punti e pesi di Gauss nel volume ee 
				Xke1(ee)=X1;
				Yke1(ee)=Y1;
				Zke1(ee)=Z1;
				Wke1(ee)=WW1;      
			...
			do qq=1,nv_hh !ciclo su volumi lato hh
				do ee=1,nv_kk !ciclo volumi lato kk
					...
					L=0.0d0;
					do ii = 1,8 ! ciclo sui punti di Gauss del tetraedro qq 
						   call norm_eps([Xhq(qq,ii),Yhq(qq,ii),Zhq(qq,ii)]-[Xke1(ee),Yke1(ee),Zke1(ee)],Neps+distt/1.0d9,dist);
						   L=L+Whq(qq,ii)*Wke1(ee)/dist; 
					enddo 
					L=L*fort_wh_wk;
					Matrix_L(hhloc,kkloc)=Matrix_L(hhloc,kkloc)+L;
				enddo 
...
subroutine fun_tetra_one_point(tetra,Vol,Xg,Yg,Zg,WWg)
implicit none
real*8 tetra(3,4), Vol, Xg, Yg, Zg, WWg
real*8 wg, p1, p2, p3, p4
wg=0.166666668
p1=0.25d0
p2=0.25d0
p3=0.25d0
...
Zg = p1*tetra(3,1)+p2*tetra(3,2)+p3*tetra(3,3)+p4*tetra(3,4)
WWG = wg*Vol*6.d0;
end subroutine fun_tetra_one_point