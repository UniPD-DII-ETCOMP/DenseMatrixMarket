function [A_eapp,A_napp,R_app,L_app,Cinv_app,Us_app]=set_VoltageSource(nn_dual,n_Vsource,nn_app,Voltage_Source)
...
for k=1:n_Vsource
    %voltage generator connected from INF node to OBJ node
    ...
    %voltage generator connected form INF node to APPENDED node    
    elseif sign(Voltage_Source(k).node_end) == -1 && sign(Voltage_Source(k).node_start) == 0
        ...
        Cinv_app(k,k)=Voltage_Source(k).Cinv;
        Us_app(k)=Voltage_Source(k).value;
    %voltage generator connected from APPENDED node to OBJ node
    elseif sign(Voltage_Source(k).node_end) == 1 && sign(Voltage_Source(k).node_start) == -1
        ...
        Cinv_app(k,k)=Voltage_Source(k).Cinv;
        Us_app(k)=Voltage_Source(k).value;
    %voltage generator connected from OBJ node to APPENDED node
    elseif sign(Voltage_Source(k).node_end) == -1 && sign(Voltage_Source(k).node_start) == 1
        ...
        Cinv_app(k,k)=Voltage_Source(k).Cinv;
        Us_app(k)=Voltage_Source(k).value;
    %voltage generator connected from OBJ node to OBJ node
    elseif sign(Voltage_Source(k).node_end) == 1 && sign(Voltage_Source(k).node_start) == 1
        A_eapp(Voltage_Source(k).node_start,k)=-1;
        A_eapp(Voltage_Source(k).node_end,k)=1;
        R_app(k,k)=Voltage_Source(k).R;
        ...
        Cinv_app(k,k)=Voltage_Source(k).Cinv;
        Us_app(k)=Voltage_Source(k).value;
    %voltage generator connected from APPENDED node to APPENDED node
    elseif sign(Voltage_Source(k).node_end) == -1 && sign(Voltage_Source(k).node_start) == -1
        A_napp(abs(Voltage_Source(k).node_start),k)=-1;
        A_napp(abs(Voltage_Source(k).node_end),k)=1;
        R_app(k,k)=Voltage_Source(k).R;
