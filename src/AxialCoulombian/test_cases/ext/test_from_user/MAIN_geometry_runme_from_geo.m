...
%%
figure
patch('Faces',F1(1:4,:).','Vertices',Matrix_P0,'Facecolor','r','FaceAlpha',0.2)
axis equal
view(0,0)
axis([min(Matrix_P0(:,1)) max(Matrix_P0(:,1)) min(Matrix_P0(:,2))-1 max(Matrix_P0(:,2))+1 min(Matrix_P0(:,3)) max(Matrix_P0(:,3))])
%% check normal vector
...
axis([min(Matrix_P0(:,1)) max(Matrix_P0(:,1)) min(Matrix_P0(:,2))-1 max(Matrix_P0(:,2))+1 min(Matrix_P0(:,3)) max(Matrix_P0(:,3))])
%%
save data_ext.mat Matrix_G_e G2_e C1_e  F1_e G1_e Matrix_C_e  Matrix_P0_e  
